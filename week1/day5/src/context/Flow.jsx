...
function Provider(props) {
	...
	const isReady =
		state.balance !== null && state.collection !== undefined;

	const fetchBalance = useCallback(async () => {
		if (
			state.user.addr &&
			state.user.addr !== '0xLocalArtist'
		) {
		...
		const transactionId = await fcl
          ...
          transaction(listingIndex: Int) {
            ...
            prepare(account: Authaccount){
              ...
              let LocalArtistMarketRef = getAccount(${process.env.REACT_APP_ARTIST_CONTRACT_HOST_ACCOUNT})
              .getCapability(/public/LocalArtistMarket)
              .borrow<&{LocalArtistMarket.MarketInterface}>()
              ?? panic("Couldn't borrow market reference.")
              ...
              LocalArtistMarketRef.buy(
                listingIndex: listingIndex,
                tokenVault: tokenVaultRef.withdraw(amount: listing.price ), 
